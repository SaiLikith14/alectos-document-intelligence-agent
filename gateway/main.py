...
# A platform answers 502/503/504 itself while an instance that has scaled to zero
# wakes up. Those responses are generated by the edge and never reached the
# application, so replaying the request cannot duplicate any work. Anything the
# app itself returns, including its own 5xx, is passed through untouched.
WAKING = {502, 503, 504}
WAKE_ATTEMPTS = 4
WAKE_DELAY = 3.0


async def waking_up(response):
    return response.status_code in WAKING


@asynccontextmanager
async def open_upstream_stream(client, method, path, **kwargs):
    """Open a streamed upstream request, riding out a cold start.

    A retry is only attempted before a single byte has been forwarded, so the
    client never sees a duplicated or partial replay. Yields the live response
    and closes it on exit.
    """
    delay = WAKE_DELAY
    for attempt in range(WAKE_ATTEMPTS):
        context = client.stream(method, path, **kwargs)
        response = await context.__aenter__()
        if not await waking_up(response) or attempt == WAKE_ATTEMPTS - 1:
            try:
                yield response
            finally:
            ...
            return
        await context.__aexit__(None, None, None)
        log.info('Upstream still waking (%s), retrying stream in %.0fs', response.status_code, delay)
        ...
        try:
            async with asyncio.timeout(config.gateway_timeout_seconds):
                ...
                for attempt in range(WAKE_ATTEMPTS):
                    ...
                    if not await waking_up(response) or attempt == WAKE_ATTEMPTS - 1:
                        break
                    log.info('Upstream still waking (%s), retrying in %.0fs', response.status_code, delay)
                    await asyncio.sleep(delay)
                    delay *= 1.6
            ...
            try:
                async with asyncio.timeout(config.gateway_timeout_seconds):
                    async with open_upstream_stream(
                        request.app.state.client, 'POST', f'/api/v1/agents/{AGENT}/query/stream',
        ...
        async def research_chat(payload: ResearchChat, request: Request, user: str = Depends(current_user),
            ...
            async def stream():
                ...
                try:
                    async with asyncio.timeout(config.gateway_timeout_seconds):
                        async with open_upstream_stream(
                            request.app.state.research, 'POST', '/api/chat/stream',
                            json=payload.model_dump()) as response:
                            if not response.is_success:
                                status = 'failed' if response.status_code < 500 else 'uncertain'
